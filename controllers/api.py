...
# Filters a frequency list for exclusions and returns the result
def filter_exclusions(freq_array):
  new_array = [obj for obj in freq_array if obj.token not in exclusions]
  return new_array
  
def add_exclusion():
  logger.debug(request.vars)
  freq_array = request.vars["freq_array[]"]
  token = request.vars.token
  exclusions.append(token)
  new_array = filter_exclusions(freq_array)
  return response.json(
    dict(
      freq_array = new_array
    )
  )
